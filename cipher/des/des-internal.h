 ...
 * des-internal.h
 *
 *  Created on: Feb 18, 2019, 4:42:27 PM
 *      Author: Joshua Fehrenbach
 */

#ifndef DES_INTERNAL_H_
#define DES_INTERNAL_H_

#include "des.h"

#define des_keymap _crypto_des_keymap
#define des_bigmap _crypto_des_bigmap

#ifdef CRYPTO_DES_QUICK
extern const uint32_t des_bigmap[0x4000];	/* 64KB table */
#else
extern const uint32_t des_keymap[0x200];	/* 2KB table */
#endif
...
#define	ENCRYPT(NAME, TEMP, LOAD, KEYMAP, SAVE)	\
						\
static inline void ATTRIBUTE(gnu_inline,always_inline,artificial) \
NAME(REGISTER uint8_t *D,				\
     REGISTER uint32_t *r,			\
