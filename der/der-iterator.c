 ...
 * der-iterator.c
 *
 *  Created on: Oct 31, 2019, 2:18:18 PM
...
/* Basic DER syntax: (reference: A Layman's Guide to a Subset of ASN.1, BER, and DER,
   ...
   After the length comes the contents. For primitive objects (c == 0),
...
static void
asn1_der_iterator_init(struct asn1_der_iterator *i, const uint8_t *input, size_t length) {
	i->buffer = input;
	i->buffer_length = length;
	i->pos = 0;
	i->type = 0;
	i->data = NULL;
	i->length = 0;
...
/* Get the type and length of the next object */
...
asn1_der_iterator_next(struct asn1_der_iterator *i) {
	...
	if (i->length & 0x80) {
		...
		i->pos += k;
		i->length = data[0];
		if (i->length == 0 || (k == 1 && i->length < 0x80)) {
			return ASN1_ITERATOR_ERROR;
		}
		for (j = 1; j < k; j++) {
...
enum asn1_iterator_result
asn1_der_iterator_first(struct asn1_der_iterator *i, const uint8_t *input, size_t length) {
	asn1_der_iterator_init(i, input, length);
	return asn1_der_iterator_next(i);
}

enum asn1_iterator_result
asn1_der_decode_constructed(struct asn1_der_iterator *i, struct asn1_der_iterator *c) {
	assert (i->type & ASN1_TYPE_CONSTRUCTED);
	return asn1_der_iterator_first(c, i->data, i->length);
}
...
/* decode DER object wrapped in a bit string */
enum asn1_iterator_result
asn1_der_decode_bitstring(struct asn1_der_iterator *i, struct asn1_der_iterator *c) {
	assert(i->type == ASN1_BITSTRING);
	/* First byte is the number of padding bits, which must be zero */
	if (i->length == 0 || i->data[0] != 0) {
		return ASN1_ITERATOR_ERROR;
	}
	return asn1_der_iterator_first(c, i->data + 1, i->length - 1);
}
...
asn1_der_get_uint32(struct asn1_der_iterator *i, uint32_t *x) {
	/* Big-endian, two's complement, minimum number of octets
	 * (except 0, which is encoded as a single octet) */
	...
	if (length > 1 && i->data[length-1] == 0 && i->data[length-2] < 0x80) {
...
int
asn1_der_get_bignum(struct asn1_der_iterator *i, mpz_ptr x, unsigned max_bits) {
	if (i->length > 1 || ((i->data[0] == 0 && i->data[1] < 0x80) ||
	...
	crypto_mpz_set_str_256_s(x, i->length, i->data);
	/* FIXME: How to interpret a max_bits for negative numbers? */
	return !(max_bits && mpz_sizeinbase(x, 2) > max_bits);
}
