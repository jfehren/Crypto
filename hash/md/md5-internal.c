 ...
 * md5-internal.c
 *
 *  Created on: Aug 26, 2019, 2:04:47 PM
...
#define ROUND(f, w, x, y, z, data, s) ( w += f(x, y, z) + data, w = ROTL32(w, s) + x )

void
_md5_compress_gen(uint32_t *digest, const uint8_t *input) {
	uint32_t data[MD5_DATA_LENGTH];
	uint32_t a, b, c, d;

	for (unsigned i = 0; i < MD5_DATA_LENGTH; i++) {
		data[i] = LE_READ_UINT32(input + i*4);
	}
	a = digest[0];
	b = digest[1];
	c = digest[2];
...
void
_crypto_md5_compress_default(uint32_t *digest, const uint8_t *input) {
	if (is_x86_64()) {
		_crypto_md5_compress_fat = &_md5_compress_x64;
	} else {
		_crypto_md5_compress_fat = &_md5_compress_x86;
	}
	_md5_compress(digest, input);
}

void _md5_compress(uint32_t*, const uint8_t*) = &_crypto_md5_compress_default;
