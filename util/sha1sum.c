 ...
 * sha1sum.c
 *
 *  Created on: Sep 4, 2019, 1:51:01 PM
...
"sha1sum [OPTIONS] [file1 file2  file3...]\n"
"Options:\n"
...
struct sha1_ctx ctx;
uint8_t hashval[SHA1_DIGEST_SIZE];

struct hash_struct hash = {
	SHA1, &ctx,
	(crypto_hash_update_func*)&sha1_update,
	(crypto_hash_digest_func*)&sha1_digest,
	hashval, SHA1_DIGEST_SIZE
};
...
int check_hash(const char *filename, const char *oldhash) {
	...
	FILE *in;
	struct stat st;
	if (encoder->decoded_size((const unsigned char*)oldhash) != SHA1_DIGEST_SIZE) {
		fprintf(stderr, "Invalid %s SHA-1 Digest: %s\n", encoder->name, oldhash);
		exit(-1);
	}
...
check_hash_err:
	...
	hash_filep(&hash, in);
	fclose(in);
	return memcmp(hashval, decoded, SHA1_DIGEST_SIZE) != 0;
}
...
int optC(int argc, const char *argv[], int i) {
	...
	char hashcheck[SHA1_DIGEST_SIZE*8+1];	/* Max length (in case of binary format) */
	FILE *checkfile;
	long fp;
	int ret;
	if (i == argc-1) {
	...
	while (1) {
		...
		if (encoder->decoded_size((const unsigned char*)hashcheck) != SHA1_DIGEST_SIZE) {
			fprintf(stderr, "Invalid SHA-1 Digest in hash check file \"%s\";"
