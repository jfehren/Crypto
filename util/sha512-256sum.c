 ...
 * sha512-256sum.c
 *
 *  Created on: Sep 12, 2019, 10:19:59 AM
...
"sha512-256sum [OPTIONS] [file1 file2  file3...]\n"
"Options:\n"
...
"\t                  in the same format as is output by sha512-256sum.\n"
"\t-h            Prints this help message, ignoring all following arguments.\n"
"\n"
"Options are case-sensitive and are processed in order.\n"
"\n"
"For each file given, sha512-256sum provides output of the form:\n"
"Name: filename\n"
"SHA-512/256-Digest: hashvalue\n"
"\n"
"When not using -c or -C, file names can include wildcards. For example,\n"
"sha512-256sum *.txt\n"
"will output the SHA-512/256 Digest of all files in the current working directory\n"
"that have a .txt extension. In addition, the contents of directories are recursively\n"
"processed as well so, for example,\n"
"sha512-256sum *\n"
"will output the SHA-512/256 Digest of all files in the current working directory and\n"
"all files in any sub-directories in the current working directory.\n"
"\n"
"In the case that no input file is given, or the file given is '-', sha512-256sum\n"
...
struct sha512_256_ctx ctx;
uint8_t hashval[SHA512_256_DIGEST_SIZE];

struct hash_struct hash = {
	SHA512_256, &ctx,
	(crypto_hash_update_func*)&sha512_256_update,
	(crypto_hash_digest_func*)&sha512_256_digest,
	hashval, SHA512_256_DIGEST_SIZE
};
...
int check_hash(const char *filename, const char *oldhash) {
	...
	FILE *in;
	struct stat st;
	if (encoder->decoded_size((const unsigned char*)oldhash) != SHA512_256_DIGEST_SIZE) {
		fprintf(stderr, "Invalid %s SHA-512/256 Digest: %s\n", encoder->name, oldhash);
		exit(-1);
	}
...
check_hash_err:
	...
	hash_filep(&hash, in);
	fclose(in);
	return memcmp(hashval, decoded, SHA512_256_DIGEST_SIZE) != 0;
}
...
int optC(int argc, const char *argv[], int i) {
	...
	char hashcheck[SHA512_256_DIGEST_SIZE*8+1];	/* Max length (in case of binary format) */
	FILE *checkfile;
	long fp;
	int ret;
	if (i == argc-1) {
	...
	while (1) {
		...
		if (encoder->decoded_size((const unsigned char*)hashcheck) != SHA512_256_DIGEST_SIZE) {
			fprintf(stderr, "Invalid SHA-512/256 Digest in hash check file \"%s\";"
