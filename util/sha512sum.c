 ...
 * sha512sum.c
 *
 *  Created on: Sep 12, 2019, 10:15:01 AM
...
"sha512sum [OPTIONS] [file1 file2  file3...]\n"
"Options:\n"
...
uint8_t hashval[SHA512_DIGEST_SIZE];

struct hash_struct hash = {
	SHA512, &ctx,
	(crypto_hash_update_func*)&sha512_update,
	(crypto_hash_digest_func*)&sha512_digest,
	hashval, SHA512_DIGEST_SIZE
};
...
int check_hash(const char *filename, const char *oldhash) {
	...
	FILE *in;
	struct stat st;
	if (encoder->decoded_size((const unsigned char*)oldhash) != SHA512_DIGEST_SIZE) {
		fprintf(stderr, "Invalid %s SHA-512 Digest: %s\n", encoder->name, oldhash);
		exit(-1);
	}
...
check_hash_err:
	...
	hash_filep(&hash, in);
	fclose(in);
	return memcmp(hashval, decoded, SHA512_DIGEST_SIZE) != 0;
}
...
int optC(int argc, const char *argv[], int i) {
	...
	char hashcheck[SHA512_DIGEST_SIZE*8+1];	/* Max length (in case of binary format) */
	FILE *checkfile;
	long fp;
	int ret;
	if (i == argc-1) {
	...
	while (1) {
		...
		if (encoder->decoded_size((const unsigned char*)hashcheck) != SHA512_DIGEST_SIZE) {
			fprintf(stderr, "Invalid SHA-512 Digest in hash check file \"%s\";"
