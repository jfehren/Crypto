 ...
 * md4sum.c
 *
 *  Created on: Sep 12, 2019, 2:41:14 PM
...
"md4sum [OPTIONS] [file1 file2  file3...]\n"
"Options:\n"
...
struct md4_ctx ctx;
uint8_t hashval[MD4_DIGEST_SIZE];

struct hash_struct hash = {
	MD4, &ctx,
	(crypto_hash_update_func*)&md4_update,
	(crypto_hash_digest_func*)&md4_digest,
	hashval, MD4_DIGEST_SIZE
};
...
int check_hash(const char *filename, const char *oldhash) {
	...
	FILE *in;
	struct stat st;
	if (encoder->decoded_size((const unsigned char*)oldhash) != MD4_DIGEST_SIZE) {
		fprintf(stderr, "Invalid %s MD4 Digest: %s\n", encoder->name, oldhash);
		exit(-1);
	}
...
check_hash_err:
	...
	hash_filep(&hash, in);
	fclose(in);
	return memcmp(hashval, decoded, MD4_DIGEST_SIZE) != 0;
}
...
int optC(int argc, const char *argv[], int i) {
	...
	char hashcheck[MD4_DIGEST_SIZE*8+1];	/* Max length (in case of binary format) */
	FILE *checkfile;
	long fp;
	int ret;
	if (i == argc-1) {
	...
	while (1) {
		...
		if (encoder->decoded_size((const unsigned char*)hashcheck) != MD4_DIGEST_SIZE) {
			fprintf(stderr, "Invalid MD4 Digest in hash check file \"%s\";"
