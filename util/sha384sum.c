 ...
 * sha384sum.c
 *
 *  Created on: Sep 12, 2019, 9:59:08 AM
...
"sha384sum [OPTIONS] [file1 file2  file3...]\n"
"Options:\n"
...
uint8_t hashval[SHA384_DIGEST_SIZE];

struct hash_struct hash = {
	SHA384, &ctx,
	(crypto_hash_update_func*)&sha384_update,
	(crypto_hash_digest_func*)&sha384_digest,
	hashval, SHA384_DIGEST_SIZE
};
...
int check_hash(const char *filename, const char *oldhash) {
	...
	FILE *in;
	struct stat st;
	if (encoder->decoded_size((const unsigned char*)oldhash) != SHA384_DIGEST_SIZE) {
		fprintf(stderr, "Invalid %s SHA-384 Digest: %s\n", encoder->name, oldhash);
		exit(-1);
	}
...
check_hash_err:
	...
	hash_filep(&hash, in);
	fclose(in);
	return memcmp(hashval, decoded, SHA384_DIGEST_SIZE) != 0;
}
...
int optC(int argc, const char *argv[], int i) {
	...
	char hashcheck[SHA384_DIGEST_SIZE*8+1];	/* Max length (in case of binary format) */
	FILE *checkfile;
	long fp;
	int ret;
	if (i == argc-1) {
	...
	while (1) {
		...
		if (encoder->decoded_size((const unsigned char*)hashcheck) != SHA384_DIGEST_SIZE) {
			fprintf(stderr, "Invalid SHA-384 Digest in hash check file \"%s\";"
